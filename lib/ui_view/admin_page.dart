...
class AdminPage extends StatefulWidget {
  const AdminPage({super.key, this.animationController});
...
class AdminPageState extends State<AdminPage> with TickerProviderStateMixin {
  ...
  Future<void> _fetchUserSection() async {
    try {
                  ...
                  Expanded(
                    child: Container(
                        ...
                        child: tabIndex == 0
                            ? (loading == true)
                                ? ShimmerCard()
                                : GetMainListViewUI(userSection: userSection!)
                            : (loading == true)
                                ? ShimmerCard()
                                : GetMainListViewUI2(
...
class GetMainListViewUI extends StatefulWidget {
  ...
  const GetMainListViewUI({super.key, required this.userSection});
...
class _GetMainListViewUIState extends State<GetMainListViewUI> {
  late final Query<Request> requestQuery;

  @override
  void initState() {
    super.initState();
    requestQuery = FirebaseFirestore.instance
        .collection('request')
        .where('approval', isEqualTo: 'Pending')
        .where('section', isEqualTo: widget.userSection) // Use user's section
...
class GetMainListViewUI2 extends StatefulWidget {
  ...
  const GetMainListViewUI2({super.key, required this.userSection});
...
class GetMainListViewUI2State extends State<GetMainListViewUI2> {
  ...
  void initState() {
    ...
    requestQuery = FirebaseFirestore.instance
        .collection('request')
        .where('approval', isNotEqualTo: 'Pending')
        .where('section', isEqualTo: widget.userSection) // Use user's section
        .orderBy("date", descending: true)
        .withConverter(
          fromFirestore: (snapshot, _) => Request.fromMap(snapshot.data()!),
